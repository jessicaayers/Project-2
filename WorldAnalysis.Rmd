...
```{r}
library(caret)
worldTrain <- worldTrain %>% select(-starts_with("data_channel"),-url)
worldTest <- worldTest %>% select(-starts_with("data_channel"),-url)
preprocess <- preProcess(worldTrain, method = c("center", "scale"))
worldTrainPreprocessed <- predict(preprocess, worldTrain)
worldTestPreprocessed <- predict(preprocess, worldTest)
```

```{r}
lmFit1<- train(shares ~ ., data = worldTrain, method = "lm",preProcess=c("center","scale"),
trControl = trainControl(method = "cv", number = 5))

summary(lmFit1)

lmFit1Pred <- predict(lmFit1, newdata = worldTestPreprocessed)
lm1 <- postResample(lmFit2Pred, worldTestPreprocessed$shares)
```
...
boosted_tree_model <- train(shares ~ ., data = worldTrainPreprocessed, method = "gbm", trControl = control, tuneGrid = tuningGrid)
...
```{r}
boosted_tree_predictions <- predict(boosted_tree_model, newdata = worldTestPreprocessed)

boosted_tree <- postResample(boosted_tree_predictions, worldTestPreprocessed$shares)
boosted_tree
```
...
lmFit2 <- train(shares ~ .^2, data = worldTrain, method = "lm",preProcess=c("center","scale"),
trControl = trainControl(method = "cv", number = 5))
...
#using cross validation
ncol(worldTrain)
rfFit <- train(shares ~ ., data = worldTrainPreprocessed, method = "rf", 
               trControl = trainControl(method = "cv", number = 5), 
